...
for snr = 1:1:20   
    ...
    BitErrorManyZhen_rake_c = 0;
    BitErrorManyZhen_norake = 0;
    BitErrorManyZhen_afterMerge = 0;
...
%         figure(2), subplot(212);plot(buHuoQuMo); title('软相关的相关峰');axis([0 2000 0 700]);
...
%             path_3(1,1:end-shift_x(3)) = path_3(1,shift_x(3)+1:end);
            shift_x = shift_x * -1;      %变负数，让其左移        
            path_1 = circshift(rx,[0,shift_x(1)]);
            path_2 = circshift(rx,[0,shift_x(2)]);
            path_3 = circshift(rx,[0,shift_x(3)]);
...
%        end
      %%
        ...
        y0Rake = abs(y4);
        yy1_1 = step(xcor,data_out1',pp1');
        yy1_0 = step(xcor,data_out1',pp0');
        y1Rake_a = abs(yy1_1);%第一路的做相关
        y0Rake_a = abs(yy1_0);
        yy2_1 = step(xcor,data_out2',pp1');
        yy2_0 = step(xcor,data_out2',pp0');
        y1Rake_b = abs(yy2_1);%第二路的做相关
        y0Rake_b = abs(yy2_0);
        yy3_1 = step(xcor,data_out3',pp1');
        yy3_0 = step(xcor,data_out3',pp0');
        y1Rake_c = abs(yy3_1);%第三路的做相关
        y0Rake_c = abs(yy3_0);
        %相关之后再合并
        corrMerge_1 = yy1_1 + yy2_1 + yy3_1;
        corrMerge_0 = yy1_0 + yy2_0 + yy3_0;
        xcor = dsp.Crosscorrelator;
       ...
       %%
        ...
        if(feng_x(2)~=1 && feng_x(3)~=1)
            ...
            BitErrorManyZhen_rake_b = BitErrorManyZhen_rake_b + BitErrorRake_b;
        end
        %--第三路 %%%%%%%%%%%%%%%%%%%%%%%%%
        if(feng_x(2)~=1 && feng_x(3)~=1)
            receiveJudge_c = ones(1,Tlen);           
            tt = 1;
            for i = T_sample-sh : T_sample : L_s_initial1 - sh + 1
                 if(sum(y1Rake_c(i : i+2*sh)) > sum(y0Rake_c(i : i+2*sh)))
                     receiveJudge_c(tt) = 1;
                 else
                     receiveJudge_c(tt) = -1;
                 end
            ...
            BitErrorRake_c = length(find(receiveJudge_c ~= s_initial));
            BitErrorManyZhen_rake_c = BitErrorManyZhen_rake_c + BitErrorRake_c;
        end
        %对比不同步不矫正                          %%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %--对软相关后的值合并 %%%%%%%%%%%%%%%%%%%%%%%%%
        if(feng_x(2)~=1 && feng_x(3)~=1)
            receiveJudge_afterMerge = ones(1,Tlen);           
            tt = 1;
            for i = T_sample-sh : T_sample : L_s_initial1 - sh + 1
                 if(sum(corrMerge_1(i : i+2*sh)) > sum(corrMerge_0(i : i+2*sh)))
                     receiveJudge_afterMerge(tt) = 1;
                 else
                     receiveJudge_afterMerge(tt) = -1;
                 end
                 tt = tt+1;
            end
            BitErrorRake_merge = length(find(receiveJudge_afterMerge ~= s_initial));
            BitErrorManyZhen_afterMerge = BitErrorManyZhen_afterMerge + BitErrorRake_merge;
        end
    end
    ...
    ErrorRate_c(xx) = BitErrorManyZhen_rake_c/(Tlen * number_zhen);
    ErrorRate_merge(xx) = BitErrorManyZhen_afterMerge/(Tlen * number_zhen);
    xx = xx + 1;
end
figure(2), subplot(211);plot(y); title('Correlated output');
...
figure(11)
semilogy(EbNo,ErrorRateNoRake,'m*-');hold on;grid;title('rake接收机');
semilogy(EbNo,ErrorRate_a,'bo-');hold on;grid;
semilogy(EbNo,ErrorRate_b,'go-');hold on;
semilogy(EbNo,ErrorRate_c,'mo-');hold on;  %画第三路的
semilogy(EbNo,ErrorRate,'r*-');hold on;
semilogy(EbNo,ErrorRate_merge,'cp-');hold on;

legend('仅扩频和软判决','rake第一路','rake第二路','rake第三路','rake三路合并','先软相关后合并');
